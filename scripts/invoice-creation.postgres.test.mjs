...
const concurrent = (query) =>
  exec('docker', args.concat('-c', query), { encoding: 'utf8' }).then(({ stdout }) =>
...
test(
  ...
  async () => {
    ...
    try {
      ...
      sql("UPDATE invoices SET status='cancelled' WHERE id='" + results[0].invoice_id + "';");
      assert.equal(JSON.parse(sql(create('race'))).invoice_id, results[0].invoice_id);
      assert.equal(
        sql("SELECT status FROM invoices WHERE id='" + results[0].invoice_id + "'"),
        'cancelled'
      );
      ...
      assert.equal(
        sql(
          "SELECT count(*) FROM invoice_schedules WHERE invoice_id='" + scheduled.invoice_id + "'"
        ),
        '1'
      );
      ...
      sql("DELETE FROM invoices WHERE id='" + deleted.invoice_id + "'");
      ...
      const keyedWithPause = exec(
        'docker',
        args.concat(
          '-c',
          "SET application_name='keyed-creation-test'; " +
            create('mixed-keyed', 'a'.repeat(64), { ...invoice, notes: 'mixed-keyed-race' })
        ),
        { encoding: 'utf8' }
      );
      ...
      const legacyInsert = exec(
        'docker',
        args.concat(
          '-c',
          `INSERT INTO invoices(user_id,business_id,invoice_number,status,currency,amount)
      ...
      assert.equal(
        sql(
          "SELECT count(*) - count(DISTINCT invoice_number) FROM invoices WHERE business_id='" +
            business +
            "'"
        ),
        '0'
      ...
      sql("DELETE FROM invoices WHERE id='" + replayed.invoice_id + "'");
      assert.throws(
        ...
        assert.equal(
          sql(
            "SELECT has_function_privilege('" +
              role +
              "', 'create_idempotent_invoice(uuid,text,text,jsonb,jsonb,integer)', 'execute')"
          ),
          'f'
        );
        assert.equal(
          sql("SELECT has_table_privilege('" + role + "', 'invoice_creation_requests', 'select')"),
          'f'
        );
      }
      assert.equal(
      ...
      sql('SET ROLE service_role; ' + create('service-role'));
      console.log(
